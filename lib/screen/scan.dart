...
import 'package:fe_expense/screen/scanexpensepage.dart';
import 'package:fe_expense/data/auth_client_wrapper.dart';
...
class _ScanPageState extends State<ScanPage> {
  ...
  Future<void> _extractText() async {
    ...
    try {
      ...
      setState(() {
        _extractedText = const JsonEncoder.withIndent(
          "  ",
        ).convert(json.decode(resultJson));
      });
    } catch (e) {
              ...
              ElevatedButton.icon(
                onPressed: () {
                  ...
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder:
                          (context) => ScanExpensePage(
                            merchant: extractedData['data']['merchant'] ?? '',
                            amount: _parseamount(
                              extractedData['data']['amount'],
                            ),
                            description:
                                extractedData['data']['category']['description'] ??
                                '',
                            ...
                            currency:
                                extractedData['data']['currency'] ?? 'VND',
                          ),
                    ),
                  );
                },
  ...
  double _parseamount(dynamic amount) {
    ...
    String amountStr = amount.toString().replaceAll(',', '');
