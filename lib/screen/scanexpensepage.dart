...
import 'package:fe_expense/controllers/scan_expense_controller.dart';
...
class _ScanExpensePageState extends State<ScanExpensePage> {
  ...
  void initState() {
    ...
    amountController.text = _controller.formatCurrency(widget.amount);
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildTextField("Tên cửa hàng", widget.merchant),
            _buildTextField(
              "Số tiền",
              _controller.formatCurrency(widget.amount),
            ...
            DropdownButtonFormField<String>(
              ...
              items: [
                ...
                DropdownMenuItem(value: 'S', child: Text('SGD')),
                DropdownMenuItem(value: 'CNY', child: Text('CNY')),
                DropdownMenuItem(value: '៛', child: Text('KHR')),
                DropdownMenuItem(value: '₭', child: Text('LAK')),
              ],
              onChanged: (currency) {
                if (currency == 'VND') {
                  if (widget.currency != 'VND') {
                    double amount = parseAmount(amountController.text);
                    ...
                    double convertedAmount =
                        amount * (exchangeRates[widget.currency] ?? 1);
                    ...
                    print(
                      "🔹 Updated Amount in Controller: ${amountController.text}",
                    );
                  }
                } else {
              ...
              onPressed: () async {
                try {
                  if (selectedCurrency != 'VND') {
                    _showSnackBar(
                      "Chỉ có thể lưu chi tiêu với tiền Việt Nam (VND).",
                    );
                    return;
                  }
